...
// F3: GraphViewPage used to infer the org from getMe().orgs[0] — always the
// user's FIRST org — instead of the org selected in the switcher, so a
// second org's graph view silently fetched the wrong org's data.
test.describe('visual graph view — org selection', () => {
  ...
  test('graph view is fetched for the selected org, not the first', async ({ page, api }) => {
    await api.loginAs(twoOrgAdmin);
    await page.addInitScript(() => {
      window.localStorage.setItem('shepherd.orgId', 'org-0002');
    });
    const pipe = pipeline({ id: 'pip-graph-1', org_id: orgB.id, name: 'graph-pipe' });
    api.seed({
      orgs: [orgA, orgB],
      schema: schemaFixture,
      pipelines: [pipe],
    ...
    await page.goto(`/pipelines/${pipe.id}/graph`);
    await page.waitForSelector('[data-testid="graph-view"]', { timeout: 10_000 });

    const calls = api.calls('VisualService/GraphView');
