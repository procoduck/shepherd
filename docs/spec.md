...
**Inventory labels** are a separate string-to-string map on the logical collector. App and org admins can add, update, or remove individual keys; readers can view them. Keys contain 1-128 bytes without whitespace or control characters; values contain at most 512 bytes without control characters. Labels survive agent polls and instance replacement. They support inventory search and grouping only: they do not modify Alloy-reported attributes, cluster ownership, pipeline matching, or served configuration. Collector details display every instance's reported attributes separately from editable labels. Inventory summaries show the latest instance's reported attributes.
...
### §D.3 — §12 (amended)
Add under pipelines: `GET /api/orgs/{org}/pipelines/{id}/revisions` [reader] (list only as of v0.5.0; revision `contents` and a `RestoreRevision` procedure are approved and scheduled — F-REVISIONS in the ledger).
