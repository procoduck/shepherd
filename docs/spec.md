...
/wizards                    — Wizard gallery (cards; v1 has one card)
/wizards/app-observability  — Wizard stepper
...
- **`deploy/Dockerfile.goreleaser`** (separate from the dev Dockerfile): does NOT compile Go — it copies the GoReleaser-built binary. Two stages: `FROM grafana/alloy:v1.12.2 AS alloy` (pin the exact fleet version) to source `/bin/alloy`, then `FROM gcr.io/distroless/static-debian12:nonroot`, `COPY --from=alloy /bin/alloy /usr/local/bin/alloy`, `COPY shepherd /usr/local/bin/shepherd`, `USER nonroot`, `ENTRYPOINT ["/usr/local/bin/shepherd"]`. Because `alloy validate` needs `/tmp`, the chart's emptyDir (§17.1) covers it — no shell, no package manager in the final image.
