...
| `docs/gateway-tier-plan.md` | **in progress** (W1 partially landed — see its §9 for the remaining scope): multi-session plan for the tenant-aware gateway tier, the self-report beacon, and signal/role enforcement — its own step ledger (§9), conformance gates (§6) and review gates (§7) |
