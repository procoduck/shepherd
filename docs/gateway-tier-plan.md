...
### Before merging: rebase onto main, then re-measure

This branch was cut **before** W2 merged, so it carries migrations 0007 and 0009-0012 with 0008
absent, and has never been built against W2's three DestinationService binding RPCs. On rebase:

- `TestEveryMutatingProcedureIsCapabilityClassified` will correctly go red until those three RPCs
  are classified — that is the enumeration test doing its job, not a regression.
- Every "**30** mutating procedures across **9** services" and "**23 of 30**" figure in the W10 and
  W11 rows, and in `internal/mcp/g14_g15_test.go`, becomes stale. The counts are exact for this
  tree today (verified by an independent review) and must be **re-measured after the rebase**, not
  adjusted by arithmetic. This is the same evidence-decay the W11 row already confesses to.
