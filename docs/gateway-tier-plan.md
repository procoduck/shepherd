...
> `docs/project-status.md` §4. Treat a workstream as not user-reachable until its surface ships. This document is the
> execution plan and per-step ledger for the work; `docs/project-status.md` remains the single live
> status document for the product as a whole and links here. Do not start a second product ledger:
...
| **Service owner (dev team)** | "Get my app's telemetry flowing"; does not know Alloy | `org-editor` fits better than either extreme did: it can author pipelines, wizards, visual builder and simulation, but not the org's structure (destinations, tenant routes, git credentials, teams, service accounts — `internal/auth/authz.go`) | **Scoped write** (W10): `org-editor` is still every pipeline in the org, not just this team's |
...
| **W10** | Teams, scoped identity, machine actors | — | Teams keyed by IdP group (extending `group_assignments`) *or* by explicit local members (0017, additive — membership is the union of the two) that can *own* pipelines; scoped write for the service-owner persona; service accounts carry both capability (propose vs apply, 0012) and a role tier (editor vs admin, 0018) for machine callers; two-part attribution. Blocks W11. |
...
### Sign-offs recorded 2026-09-11

Taken one gate at a time with the evidence in front of the signer; each answer is the settled
position unless a later dated entry here changes it.

- **R1 — signed.** Default prefix format is **slug + suffix** (D9 stands). The URL-as-identifier
  framing is accepted as the security model: Shepherd enforces the CORS origin allowlist and
  rotation; request rate limiting belongs at the gateway/ingress and is not enforced by Shepherd —
  the user-facing docs must say so when tenant routes ship. A route that fails to attach to an
  operator-owned gateway is surfaced as a refusal (pending and refused both fail the apply) —
  confirmed.
- **R2 — signed.** The stored set (per token and instance: component name, healthy boolean,
  timestamps; nothing else, pinned by the reflection test) is accepted; the fixed 5-minute sweep
  is accepted as-is (not made configurable); the abuse surface (Basic auth per agent token, 1 MiB
  body cap, 2 req/s burst 10 per token) is accepted. The beacon's on-by-default posture with the
  `server.beacon_disabled` off-switch is therefore signed, not merely tolerated.
- **R3 — open; build scheduled.** Nothing to sign yet: the receiver tier is a library the chart
  does not deploy. Decision: keep the gate and build the tier — receiver Deployment, its
  NetworkPolicy making the gateway the only ingress, a tested off-switch, and the pass-through
  tenancy proof against a running Alloy — then bring R3 back. Tracked in the ledger.
- **R6 — signed, conditional.** Agent proposals must **never** reach the auto-applying `gitsync`
  path (settled). Before the MCP binary is reachable by a non-human: (1) a server-side
  **per-service-account request rate limit**, and (2) `propose_pipeline_revision` records a
  distinct **`pipeline.propose` audit event** carrying the machine identity and the on-behalf-of
  human. Re-checked 2026-09-15: (2) already exists — `ValidatePipeline` writes `pipeline.propose`
  for every service-account caller (`internal/mgmtapi/rpc_pipeline.go`, red-run in
  `attribution_test.go`, since 2026-08-22) and `propose_pipeline_revision` composes that RPC — so
  only (1) remains. A sandbox-run budget was considered and not required.

### The beacon reaches real deployments at v0.0.2 — and R2 was unsigned until 2026-09-11
...
9. **Actions minutes are not billed on this public repository, but they are never free
   of wall-clock and signal.** GitHub runs every job separately, so a workflow's parallel
   jobs multiply. Before adding a job or a schedule, state its per-run job-minutes and what
   it buys; prefer a paths filter or an existing job
...
| W8 wizard catalog fan-out | **done** | G6 ✅ | Five catalog wizards — cluster-metrics, pod-logs, database, blackbox, self-monitoring — plus the existing app-observability. **G6 is enforced structurally rather than per wizard**: `Register` is the only path onto the default registry and always wraps in `roleEnforced`, whose `Commit` derives the generated content's real signals and checks them before any caller sees a result, so a future wizard that forgets to self-check is still covered. Each wizard's declared component/attribute paths are checked against the pinned artifact AND against a committed golden, and goldens are validated by the real pinned Alloy binary (Fail, never Skip). Two defects found by review and fixed here: (1) the app-observability role dropdown offered `logs`, which its unconditional `prometheus.scrape` block can never satisfy — a dead end dressed as an option; removed, and the spec `"every offered role is satisfiable"` now requires every offered role to be committable by some valid state, which generalizes past the one bad value. (2) Wrapping every wizard in `roleEnforced` turned `RenderWizard`'s preview of syntactically invalid output from 200-with-diagnostics into a 400, because `signals.Derive` returns an error for unparseable text. `signals.ErrParse` now makes that case distinguishable and role enforcement passes it through to Stage 1, which reports it with line-level diagnostics; a second test pins that the exemption did not widen into ignoring real mismatches. **Correction, 2026-08-22 (found by a browser walkthrough of a real deployment):** this row said "awaiting UI", which understated it — the five catalog wizards were not registered in the **backend** either. A wizard package registers itself in `init()`, and nothing imported them, so `ListWizards` returned exactly one wizard and the packages could never appear at runtime. Their own suites passed throughout, because a package's tests import that package by definition: they proved themselves in isolation while being absent from the product. Blank imports added, `ListWizards` now returns 6, and `TestEveryWizardPackageIsRegistered` walks `internal/wizard/` and fails on any package that is not registered (red-run proven). **UI closed the same day**: the catalog now renders whatever `ListWizards` returns, and one generic `/wizards/$kind` runner replaces the per-wizard page — the backend already returned a full schema (steps, fields, types, options), so a wizard needed no bespoke UI, only a page pointed at its kind. `Schema.Description` was added so a wizard describes itself instead of the UI keeping a parallel map of kinds it recognises; that map, and the filter that dropped every kind missing from it, is what made five registered wizards invisible. Verified in the live cluster: Pod Logs renders its four steps with no UI code written for it. **W8 is now `done`.** |
...
| W10 teams + scoped identity | **gated** (awaiting the service-accounts UI; teams + members UI shipped v0.3.0) | G11 ✅, G12 ✅, G13 ✅ · R5 ✅ | Migration 0012: `teams` (org-scoped, keyed by IdP group — the same trust root `group_assignments` uses, extended up to org level rather than a parallel permission system; **0017, additive**, drops that requirement and lets a team carry explicit local members instead of or alongside a group, membership being the union of the two — teams now work without an IdP), `pipelines.owner_team_id` (`ON DELETE SET NULL`, deliberately the opposite of 0008's RESTRICT: ownership is an administrative label, so deleting a team must neither delete the pipelines nor become impossible), `service_accounts` with a `capability` CHECK mirroring the Go constants (propose vs apply — *may this write at all*), and, since **0018** (this remediation's W3), a `role` CHECK of its own (editor vs admin, default `editor`, mirroring the human org-editor/org-admin ladder — *what may it reach once there*): before this column existed, `authorizeServiceAccountProcedure` checked only `sa.OrgID == orgID` for every non-app-admin requirement, so an apply-capability service account reached every org-admin procedure (`RotateTenantRoute`, `DeleteTeam`, `AddTeamMember`, `DeleteCredential`, `ListAudit`...) it was never explicitly granted; `role='admin'` is never assigned implicitly, only when `CreateServiceAccount` is asked for it. Also `audit_log.on_behalf_of`. **G11**: `auth.AuthorizeOwnership` is called per handler; a team member can write what their team owns and gets 403 on another team's pipeline in the same org; unowned stays admin-only. **G12** — the gate whose own wording warns against a chokepoint — is asserted per write path: all 40 mutating procedures across 10 services call `requireWriteAuthorized` inline (re-measured 2026-09-11, up from 34/9: `go test ./internal/mgmtapi -run TestEveryMutatingProcedureIsCapabilityClassified -v` — the descriptor-driven count grew with the RPCs this remediation and earlier sessions added; `TestNoToolReachesAMutatingProcedure` in `internal/mcp` mirrors the same enumeration byte-for-byte and was not independently re-run this session, since its companion G14 red run needs a live Postgres instance), and `TestEveryMutatingProcedureIsCapabilityClassified` derives the write set from the **live proto descriptors** rather than from the table it checks, so a newly added mutating RPC fails until someone classifies it. The independent reviewer enumerated the RPCs from the .proto files, confirmed the REST shim cannot carry a service-account identity at all, and red-ran two unsampled paths (`GitOpsService.CreateRepoLink`, `TenantRouteService.CreateTenantRoute`) to check the pattern generalizes past the three the suite exercises. **G13**: review found a real defect — `Shepherd-On-Behalf-Of` was copied verbatim into the audit row with no verification, so any apply-capability credential could attribute a write to any human it cared to name, including an org admin who never touched the system. **Rejecting an absent claim and verifying a present one are different guarantees, and only the first had been built.** The claim is now checked against `service_accounts.created_by` — the human recorded when an authenticated admin issued the credential, the one identity in the exchange the caller cannot influence — and two shipped tests that had asserted the vulnerable behaviour were corrected to model a real delegation. Red-run proven. **R5 resolved 2026-08-22**: a service account may not call `CreateServiceAccount` at all — issuing a credential grants identity, and machine writes are checked against `created_by` precisely because a human put their name there; a machine-minted credential would carry `svcacct:<parent>` and the chain would stop bottoming out at anyone accountable. **Known limits, deliberate**: one credential delegates for exactly one human (shared CI needs an explicit allow-list, additive); and an apply credential is an org-level power — team ownership does not further restrict machine writes, since resolving the delegating human's group membership without a session is new scope. The REST shim is stricter than the RPC path rather than looser, recorded in `router.go` rather than silently aligned. |
| W11 agent interface (MCP) | **R6 signed conditionally 2026-09-11** (per-service-account rate limit still to build; the `pipeline.propose` audit row has existed since 2026-08-22) | G14 ✅, G15 ~ · R6 ✅ (conditional) | `internal/mcp` exposes 12 tools over the existing Connect API plus `cmd/shepherd-mcp`. **G14's real enforcement is the CREDENTIAL, not the tool list** — and that distinction is the whole point. The MCP server authenticates as a propose-capability service account, so W10's `requireWriteAuthorized` refuses every mutating handler inline; `TestG14_ProposeCredentialRefusedOnEveryLiveMutatingProcedure` proves it by calling all 34 live-descriptor-enumerated mutating procedures **directly over the wire, bypassing the tool layer**, and getting 403 from each. A tool that doesn't exist yet, calling `CreatePipeline` directly, would still be refused. The static half (`TestNoToolReachesAMutatingProcedure`, deriving the mutating set from live proto descriptors) is defense in depth, not the safety. Red run re-measured on the merged tree (2026-08-22): disabling the capability check flips **25 of 34**; the other 7 are AdminService, blocked earlier by the unconditional "no service account is ever app-admin" rule. **Re-measured 2026-09-11 (this remediation, G12's half only)**: the same enumeration now counts **40 mutating procedures across 10 services** (`go test ./internal/mgmtapi -run TestEveryMutatingProcedureIsCapabilityClassified -v`) — the growth is service-account role-tier plumbing and RPCs this remediation and W2's destination-binding rebase added since 2026-08-22. `internal/mcp/registry_test.go`'s `TestNoToolReachesAMutatingProcedure` mirrors that enumeration byte-for-byte, so its total moved identically; the runtime flip-count above (25 of 34) requires `TestG14_ProposeCredentialRefusedOnEveryLiveMutatingProcedure` against a live Postgres instance and was **not** re-run this session (docs-only worktree, no Docker) — treat it as dated to 2026-08-22, not current. **G15 is partly met and marked `~` deliberately**: the round-trip is proven end to end (propose → human applies through their own session → audit shows both actors, machine row with verified on-behalf-of and human row with none), and it reuses the existing `pipeline_revisions` path rather than inventing a second proposal object. But the audited propose step in that test is `create_sandbox_run`; `propose_pipeline_revision` — the tool actually named for this — composes `ValidatePipeline` + `PreviewMatches`, neither of which writes an audit row, so **an agent proposing a change to raw Alloy text leaves no trace that the proposal was made**. Disclosed in `propose.go` and left for R6 rather than papered over: adding audit to two read endpoints would also log every UI validation. **Finding folded in from review**: verifying the on-behalf-of claim inside `requireWriteAuthorized` covered apply-gated writes only. Propose-safe procedures never call it, yet `SimulateService.CreateRun` still stamps the claim into an audit row — so W10's fix left exactly one path able to record an unverified human. Verification now runs in the auth interceptor, where the claim enters, for every procedure; red-run proven against that quieter path. Also corrected: this test's own recorded red-run evidence said "20 of 27" when the tree gives 23 of 30 — evidence written from memory instead of re-run is the failure §8 rule 2 exists to prevent. **R6 partly resolved 2026-08-22**: an agent's proposal is now audited — `ValidatePipeline` writes a `pipeline.propose` row carrying both actors **when the caller is a machine identity**, while a human validating in the authoring UI writes nothing, because logging every keystroke produces a trail nobody reads. Red-run proven, including the human-writes-nothing half. **Not adopted, recorded as a finding**: the GitOps path is not a review-then-apply mechanism — `gitsync` applies `source="git"` pipelines automatically on every poll with no human gate, so routing agent proposals through it would silently auto-apply unreviewed agent changes. |

## 10. What building this taught us

### 2026-09-15 — what the kind dev stack taught us about the chart's own route

- **The conformance suite never routed to Shepherd itself.** It proves Gateway API + NGINX
  Gateway Fabric against the receiver tier, so the chart's own `route.enabled` HTTPRoute — the one
  an operator actually installs — had never been exercised through NGF. The first `make dev-kind`
  showed it could not work: the Service hardcoded `appProtocol: kubernetes.io/h2c`, and NGF refuses
  to proxy an HTTP route to an h2c upstream (`ResolvedRefs=False / UnsupportedProtocol`). The
  hint is now the `service.appProtocol` value (default unchanged), cleared for NGF. The rule from
  AGENTS.md applies once more: verify the control at the layer it is consumed at — here, the
  Route the chart renders, through the controller the chart targets.
...
### 2026-08-22 — what a browser walkthrough of a real deployment found

Two kind clusters (Shepherd + a separate client cluster running real Alloy against it), the chart
installed as an operator would install it, and every page walked in a browser. Three defects that
the whole test suite, two adversarial review rounds and a release review had all missed:

- **`alloy validate` could never run in any container we ship.** The binary is copied into the
  image and the image is `distroless/static`, which has no dynamic loader — so a dynamically
  linked binary lands there unrunnable, failing with a "no such file or directory" that names the
  loader rather than the file. Stage 2 of the validation gate was therefore dead in every
  containerised deployment, and the repo had already solved this for the *simulator* images
  (`DISTROLESS_BASE_IMAGE`) without applying it to the main one. **Nothing caught it because the
  two ways of being wrong look different**: the chart sets `alloy_binary`, so a Helm install
  hard-fails every save; dev and compose leave it empty, which SKIPS Stage 2 and reports valid.
  A test would have had to run the shipped image *and* configure the binary. `make
  check-alloy-runnable` now does exactly that at build time.
- **Five of six wizards were not in the product.** A wizard registers itself in `init()`, and
  nothing imported the W8 packages, so `ListWizards` returned one. Their suites passed the whole
  time — a package's tests import that package, so it proves itself in isolation while being
  absent from the binary. This is the "fixtures testing themselves" shape at package granularity,
  and the ledger's "awaiting UI" hid it.
- **Two-part attribution was invisible.** `on_behalf_of` was stored, returned by the API, and
  never rendered. G13 exists so a human reading the audit log can see who authorised a machine's
  write; a value only reachable by curl does not do that.

The common thread: **every one of these passes a unit test and fails a user.** Each control was
verified at the layer it was written at, and never at the layer it is consumed at. The kind suite
installs the chart but asserts health, not behaviour; the receiver tests validate against a docker
shim rather than the shipped image; the wizard tests import their own package. Walking the product
as an operator is not a nicer way to run the same checks — it is the only check that crosses those
seams.
