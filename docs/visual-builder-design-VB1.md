...
1. Shepherd renders the graph, then applies the **simulation transform** — a graph-level rewrite (this is WHY simulation is graph-native and near-impossible to do safely on raw text):
   - Every **Destination** node's endpoint bindings/URLs are rewritten to the harness's capture receivers (a real `prometheus.remote_write` receiver, a Loki push receiver, an OTLP receiver — the harness records everything and serves it back). Secret bindings are DROPPED in the rewrite (no real credentials ever enter the sandbox; the capture endpoints are unauthenticated inside the pod).
